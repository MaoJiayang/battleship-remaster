...
| :--- | :--- | :--- | :--- | :--- |
| **主炮** | `GUN` | **1格** | **3** (BB存活)<br>**2** (SS/CL存活)<br>**1** (其他) | **单点判定**：<br>`Target.HP -= Dmg`<br>若 `Target.HP <= 0`，状态转为 **X (毁坏)**；<br>若 `Target.HP > 0`，状态转为 **Hit (受损)**。 |
...
| **SS** | 潜艇 | 1 | **2** | **主炮/水听** | **穿甲雷击**：存活时主炮伤害=2；<br>**侦查辅助**：存活时可用水听 |
| **DD** | 驱逐舰 | 2 | **1** | **水听** | **侦查核心**：存活时可用水听。 |
...
v3.1 新增**对称推演**与**风险感知**机制：
- **对称推演**：AI 利用相同的置信状态框架模拟玩家视角，预测玩家可能的攻击行为
- **多步推演**：向前推演 k 步（默认 3 步），累积评估己方各船的沉没概率
- **风险感知**：当关键船只受威胁时，AI 会优先使用即将丧失的能力（如 CV 受威胁时多用空袭）
- **新增参数**：`riskAwareness`（风险意识，0~1）控制风险感知强度

#### 1. 核心机制：置信状态 (BeliefState)
...
| 档位 | `alpha` | `randomness` | `riskAwareness` | 特性描述 |
| :--- | :--- | :--- | :--- | :--- |
| **EASY** | 0.1 | 0.5 | 0.0 | 高随机性，经常犯错，不考虑风险 |
| **NORMAL** | 0.4 | 0.25 | 0.3 | 适度随机，偶有失误，轻微风险意识 |
| **HARD** | 0.7 | 0.0 | 0.6 | 完全理性，高度重视己方风险 |

**参数说明**：
- `alpha`：探索权重 (0~1)，值越高越倾向于信息收集，值越低越倾向于直接伤害
- `randomness`：随机决策概率 (0~1)，每回合有此概率放弃理性决策
- `riskAwareness`：风险意识强度 (0~1)，值越高越重视己方舰船的生存风险

#### 3. 决策流程 (Decision Loop)

1. **资源检测** (`aiTurn` Step1)：
    - 根据敌舰存活情况决定主炮伤害 `aiAPDamage`（BB存活则 3 点，SS/CL 存活则 2 点，否则 1 点）。
    - 若 `CV` 仍在解锁空袭；若 `DD` 或 `SS` 仍在解锁水听。
2. **概率推断** (`aiTurn` Step2)：
    - 以 `probabilityGrid` 作为主炮和空袭的基础收益。
    - 计算全图得分统计 `scoreStats`（均值、方差、归一化熵）。
    - 归一化熵的计算遵循 `evaluateScoreStats()`，先以所有可攻击格的得分组成集合 `scores`：

    ```text
    score = P(r,c) * aiAPDamage
    mean = (Σ score) / N
    variance = (Σ (score - mean)^2) / N
    取正得分集合 weights = { score | score > 0 }
    total = Σ weights
    p_i = weight_i / total
    entropy = - Σ (p_i * ln p_i)
    normEntropy = entropy / ln(|weights|)
    ```

    - 其中 `maxScore` 为当前最高打击收益；`normEntropy` 越接近 1 代表分布越平均（缺乏突破口），越接近 0 则说明存在明显高点，可直接主炮追打。
...
4. **理性分支-武器决策**：
    ...
    - **水听 (SONAR)**：评估每个 3x3 区域的未知覆盖率 `coverage = unknown/9` 与密度 `density = mass / unknown`，并记录未知格数量 `unknownCount`。当 `unknownCount >= L_max`（`L_max` 为玩家当前尚未沉没船只的最大长度）且 `1 - scoreStats.normEntropy < sonarEntropyGate`（即“热点集中度”过低）时触发侦查。
