...
## 武器系统解耦设想
- 引入 `Weapon` 抽象接口：暴露 `canUse(context)`, `previewArea(target)`, `resolve(target, context)` 等统一方法，所有武器实现该接口，负责自身冷却/依赖判定与实际地图效果。
- 建立 `WeaponService`/`WeaponRegistry`：集中完成武器实例注册、根据玩家或 AI 请求分发武器对象，并负责记录当前选中武器、事件订阅等状态；UI/AI 只与该服务交互。
- 战斗上下文（己方/敌方网格引用、日志、动画钩子）通过 `context` 参数下发，避免武器内部直接依赖全局变量；同时便于未来复用到双人或联网模式。
- 将地图更新与动画效果拆分：武器执行 `resolve` 时只返回结构化结果（命中格、日志、特效标记），由统一的 `BattleRenderer`/`EffectBus` 解释这些结果，降低 UI 与武器的耦合。
- 当新增武器时，仅需：在数据层添加配置 -> 实现接口 -> 注册到 `WeaponRegistry`；其余系统通过统一接口获得新武器能力，避免再次修改核心控制流。